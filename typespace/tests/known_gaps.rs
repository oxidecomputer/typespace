...
// The declared optional-nullable wrapper's Default provision is never
// consulted, so a wrapper narrowed below `Default: Always` still gets
// a `derive(Default)` its declaration says cannot compile.
//
// The graph, roughly:
//
//     struct Holder { maybe: Opt<u32> }    (optional and nullable)
//
// with the wrapper declared `Default: Never`: the wrapper crate has no
// Default impl and the declaration says so. `feasibility`'s
// struct-Default arms in `trait_resolution.rs` exempt every optional
// property, on the ground that it renders as an Option or as the
// configured wrapper, taken to be Default whatever it holds. That
// holds for std Option and for the `ContainerType::option()` preset,
// but `with_provision` can narrow Default to `IfParameters` or
// `Never`, and neither phase consults the narrowed declaration at this
// edge for Default: required resolution absorbs Default at Holder with
// an empty obligation list, and the desired phase's `provides` reads
// the same empty list. Rendering then puts Default among Holder's
// derives (the early return in `default_impl`, `build/structs.rs`)
// although the field's type `::custom::Opt<u32>` has no Default impl,
// and the output does not compile. The rendered field also carries a
// bare `#[serde(default)]` (`render_struct_property`'s CustomType arm,
// `lib.rs`), which needs the same missing impl on every Deserialize
// path, so a wrapper without an unconditional Default is broken more
// widely than this trait-resolution gap; where that leaves the
// narrowed declarations is a design question.
#[test]
#[ignore = "the wrapped edge is exempt from Default in both phases"]
fn desired_default_respects_the_wrapper_declaration() {
    let declaration = ContainerType::option()
        .with_path("::custom::Opt")
        .with_provision(TypespaceTrait::Default, TraitProvision::Never);
    let settings = Settings::minimal()
        .with_optional_nullable(OptionalNullable::CustomType(declaration))
    ...
    assert!(
        !derives.iter().any(|name| name == "Default")
            && !common::has_impl(&file, "Default", "Holder"),
        "Default was granted through a wrapper declared never to \
         provide it; output:\n{}",
        prettyplease::unparse(&file)
    );
}

// The required-phase side of the same gap: a Default required of every
// type should conflict at the wrapper, naming it, rather than pass.
#[test]
#[ignore = "the wrapped edge is exempt from Default in both phases"]
fn required_default_conflicts_at_a_wrapper_without_default() {
    let declaration = ContainerType::option()
        .with_path("::custom::Opt")
        .with_provision(TypespaceTrait::Default, TraitProvision::Never);
    let settings = Settings::minimal()
    ...
    assert!(
        matches!(result, Err(Error::TraitConflicts { .. })),
        "a required Default passed a wrapper declared never to provide it"
    );
}
